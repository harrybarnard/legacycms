...
class Zend_View_Helper_EDate
{
    ...
    public function EDate($startdate,$enddate)
    {
        ...
        $start = date('d/m/Y H:i',$fstart); // create formatted start date from timestamp 
        ...
        if($enddate != NULL) : // If and end date is set
            ...
            if($sstart == $send) : // If the start and end dates match
                $end = ' - '.date('H:i',$fend); // create formatted end time
            else :
                $end = ' - '.date('d/m/Y H:i',$fend); // create formatted end date
            endif;
